...
#app.add_middleware(
    #CORSMiddleware,
   # allow_origins=["http://localhost:5173","http://localhost:3000", "http://34.46.121.105"],
  #  allow_methods=["GET"],
 #   allow_headers=["*"],
#)
import os

# Comma-separated origins from env; falls back to permissive for the demo
...
@app.get("/api/health")
def get_health():
    """Latest cluster snapshot, pulled from the newest successful status read."""
    conn = get_db_connection()
    with conn, conn.cursor() as cur:
        cur.execute("""
